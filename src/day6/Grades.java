package day6;

import java.util.Scanner;

public class Grades {
    private  static Scanner scanner = new Scanner(System.in);
    static void main() {

        int[] grades = getGrades();
        printGrades(grades);
        calculateAvg(grades);
        System.out.println("The average score is " + calculateAvg(grades));
        System.out.println("The highest score is " + getHigthest(grades));
        System.out.println("The lowest score is " + getLowest(grades));
        scanner.close();
    }

    public static int[] getGrades() {
        int[] grades;
       // Scanner scanner = new Scanner(System.in);
        System.out.println("How many grades you want to enter?");
        int length = scanner.nextInt();
        // grades = new int[length];
        grades = new int[scanner.nextInt()];
        for (int i = 0; i < grades.length; i++) {
            System.out.println("Please enter grade " + (i + 1));
            grades[i] = scanner.nextInt();
        }
        return grades;
    }

    public static void printGrades(int[] grades) {
        for (int grade : grades) {
            System.out.println(grade);
        }
    }

    public static double calculateAvg(int[] grades) {

        return sum(grades) / grades.length;
    }

    public static int sum(int[] grades) {
        int sum = 0;
        for (int grade : grades) {
            sum += grade;
        }
        return sum;
    }

    public static int getHigthest(int[] grades) {
        int highest = grades[0];
        for (int grade : grades) {
            if (grade > highest) {
                highest = grade;
            }
        }
        return highest;
    }

    public static int getLowest(int[] grades) {
        int lowest = grades[0];
        for (int grade : grades) {
            if (grade < lowest) {
                lowest= grade;
            }
        }
        return lowest;
    }
}
